...
namespace ai_it_wiki.Controllers
{
  ...
  public class LLMOpenAPIController : OpenApiControllerBase
  {
    ...
    ///<summary>
    /// Получить краткую информацию о всех товарах: SKU, имя, product_id, описание категории, рейтинг.
    /// </summary>
    /// <remarks>
    /// Метод всегда возвращает полную структуру без параметров.
    /// </remarks>
    [HttpGet("products/short-info")]
    [SwaggerOperation(
        Summary = "Краткая информация о товарах",
        Description = "Возвращает список объектов с полями: sku, name, id, descriptionCategoryId, rating."
    )]
    [SwaggerResponse(StatusCodes.Status200OK, "OK", typeof(ChunkedResponseDto))]
    [ApiExplorerSettings(GroupName = "LLMOpenAPI")]
    public async Task<IActionResult> GetShortInfo()
    {
...

      {
        ...
        Filter = new ProductInfoListFilter(),
      ...
      var productListItems = await _ozonApiService.GetProductsAsync(request, CancellationToken.None);



      var ids = productListItems.Select(p => p.ProductId).ToList();

      var productInfoListRequest = new ProductInfoListRequest
      ...
      var productInfoResponse = await _ozonApiService.GetProductInfoListAsync(
      ...
      // Получаем список sku из ответа

      var skus = productInfoResponse.Items?.Select(i => i.Sku).ToList() ?? new List<long>();

      // Получаем рейтинг по SKU

      var ratingRequest = new RatingRequest { Skus = skus };
      ...
      var result = productInfoResponse.Items?.Select(item => new

      {
        ...
        Rating = ratingResponse.Products?.FirstOrDefault(r => r.Sku == item.Sku)?.Rating
...
>>>>>>> 5ebac6bfdb0204d6444ee02d344be3f37383f09d
