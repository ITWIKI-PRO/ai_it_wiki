...
namespace ai_it_wiki.Controllers
{
  ...
  public class LLMOpenAPIController : Controller
  {
    ...
    )
    {
      ...
      try
      {
        var result = await _ozonApiService.GetProductInfoListAsync(productInfoListRequest, cancellationToken);
        return Ok(result);
      }
      catch (Exception ex)
      {
      ...
      switch (data)
      {
        ...
        case ProductInfoListResponse p:
          ...
          var projItems = items.Select(it => new Dictionary<string, object?>()
          {
            ...
            ["product_id"] = fieldSet.Contains("items.product_id") || fieldSet.Contains("product_id") ? it.Id : null,
            ...
            ["attributes"] = fieldSet.Contains("items.attributes") || fieldSet.Contains("attributes") ? it : null,
          })
          .Select(d => d.Where(kv => kv.Value != null).ToDictionary(kv => kv.Key, kv => kv.Value))
          .ToList();
