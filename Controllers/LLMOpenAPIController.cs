...
namespace ai_it_wiki.Controllers
{
    ...
    public class LLMOpenAPIController : ControllerBase
    {
        ...
        [SwaggerOperation(
            ...
            OperationId = "LLM_GetOzonProducts"
        )]
        ...
        )
        {
            ...
            try
            {
                var result = await _ozonApiService.GetProductInfoListAsync(request, cancellationToken);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        [SwaggerOperation(
            Summary = "Получить список товаров (по запросу)",
            Description = "Принимает ProductInfoListRequest и возвращает объекты Ozon API",
            OperationId = "LLM_SearchOzonProducts"
        )]
        [SwaggerResponse(StatusCodes.Status200OK, "OK", typeof(ProductInfoListResponse))]
        [ProducesResponseType(typeof(ProductInfoListResponse), StatusCodes.Status200OK)]
        public async Task<IActionResult> SearchProducts(
            [FromBody] ProductInfoListRequest request,
            CancellationToken cancellationToken
        )
        {
            if (request == null)
    ...
    )
    {
      ...
      catch (Exception ex)
      {
        ...
        {
            if (string.IsNullOrWhiteSpace(fields)) return data;

            var fieldSet = new HashSet<string>(fields.Split(',').Select(f => f.Trim()), StringComparer.OrdinalIgnoreCase);

            // Простая реализация для известных типов
            switch (data)
            {
                case RatingBySkuResponse r when r.Result != null:
                    if (fieldSet.Contains("result")) return r; // уже всё, если просят целиком

                    var projected = r.Result.Select(item => new Dictionary<string, object?>()
                    {
                        ...
                        ["improve_attributes"] = fieldSet.Contains("result.improve_attributes") || fieldSet.Contains("improve_attributes") ? item.ImproveAttributes : null,
                        ["improve_at_least"] = fieldSet.Contains("result.improve_at_least") || fieldSet.Contains("improve_at_least") ? item.ImproveAtLeast : null,
                    })
                ...
                case ProductInfoListResponse p:
                    ...
                    if (fieldSet.Contains("items")) return p;

                    var projItems = items.Select(it => new Dictionary<string, object?>()
                    {
                        ["offer_id"] = fieldSet.Contains("items.offer_id") || fieldSet.Contains("offer_id") ? it.OfferId : null,
                        ["product_id"] = fieldSet.Contains("items.product_id") || fieldSet.Contains("product_id") ? it.ProductId : null,
                        ["sku"] = fieldSet.Contains("items.sku") || fieldSet.Contains("sku") ? it.Sku : null,
                        ["name"] = fieldSet.Contains("items.name") || fieldSet.Contains("name") ? it.Name : null,
                        ["description_category_id"] = fieldSet.Contains("items.description_category_id") || fieldSet.Contains("description_category_id") ? it.DescriptionCategoryId : null,
                        ["attributes"] = fieldSet.Contains("items.attributes") || fieldSet.Contains("attributes") ? it.Attributes : null,
                    })
                    .Select(d => d.Where(kv => kv.Value != null).ToDictionary(kv => kv.Key, kv => kv.Value))
                    .ToList();
