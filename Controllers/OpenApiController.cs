...
namespace ai_it_wiki.Controllers
{
  ...
  public class OpenApiController : OpenApiControllerBase
  {
    ...
    [HttpGet("OpenUrl")]
    public IActionResult OpenUrl([FromQuery] string url, int timeOut = 10, int part = 1, ContentType contentType = ContentType.Text)
    {
        ...
        try
        {
          ...
          wait.Until(driver =>
          {
            try
            {
              ...
              bool areScriptsCompleted = (bool)((IJavaScriptExecutor)driver).ExecuteScript(@"
    ...
    {
      var text = System.IO.File.ReadAllText(Path.Combine("wwwroot", "samples", "5716.txt"), System.Text.Encoding.UTF8);
      return SplitedResponse(text, part);
    }

    [HttpPost("execute")]
    public async Task<IActionResult> ExecuteRequest([FromBody] HttpRequestModel requestModel, [FromQuery] int part = 1)
    {
      // Создание HTTP запроса в зависимости от типа метода
      var requestMessage = new HttpRequestMessage(new HttpMethod(requestModel.Method), requestModel.Url);

      // Добавление тела запроса, если есть
      if (!string.IsNullOrEmpty(requestModel.Body))
      {
        requestMessage.Content = new StringContent(requestModel.Body, System.Text.Encoding.UTF8, "application/json");
      }
