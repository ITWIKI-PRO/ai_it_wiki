...
public class OpenApiControllerBase : ControllerBase
{
  ...
  public IActionResult SplitedResponse(string text, int part)
  {
    ...
    if (tokensCount > 7000)
    {
      ...
      return new JsonResult(new { is_consequential = true, content = responseParts[part - 1], part, total_parts = responseParts.Count });
    }
    return new JsonResult(new { is_consequential = false, content = text, part = 1, total_parts = 1 });
  }
}
