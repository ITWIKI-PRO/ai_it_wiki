...
namespace ai_it_wiki.Services.Ozon
{
    public class OzonApiService : IOzonApiService
    {
        ...
        public async Task<ProductInfoListResponse> GetProductInfoListAsync(
            ProductInfoListRequest requestModel,
            CancellationToken cancellationToken = default
        )
        {
            using var request = new HttpRequestMessage(HttpMethod.Post, "/v3/product/info/list")
            ...
            try
            {
                ...
                var env = JsonSerializer.Deserialize<ProductInfoListEnvelope>(content);
                if (env?.Result != null)
                {
                    return env.Result;
                }

                // Some integrations might return the object at top-level
                var direct = JsonSerializer.Deserialize<ProductInfoListResponse>(content);
                if (direct != null)
                {
                    return direct;
                }
